...
describe('flowSchema (composed signal-forms logic)', () => {
  ...
  it('applies backend username max length', () => {
    const { model, form } = build();
    model.update((m) => ({ ...m, account: { ...m.account, username: 'a'.repeat(21) } })); // > 20
    expect(form.account.username().errors().some((e) => e.kind === 'maxLength')).toBe(true);
  });

  it('rejects a malformed profile email', () => {
    const { model, form } = build();
    model.update((m) => ({ ...m, profile: { ...m.profile, email: 'not-an-email' } }));
    expect(form.profile.email().invalid()).toBe(true);
    model.update((m) => ({ ...m, profile: { ...m.profile, email: 'tommy@example.com' } }));
  ...
  it('does not report a password mismatch when both fields are empty', () => {
    const { form } = build();
    expect(
      form.account.confirmPassword().errors().some((e) => e.kind === 'passwordMismatch'),
    ).toBe(false);
  });
});
